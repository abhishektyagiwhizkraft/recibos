                        ...
                        <div class="col-md-7 mt-2">
                          <select name="products[1][id]" id="products" class="form-control sel_pro customselect2" required=""> 
                              <option value="" disabled="" selected="">Seleccione Productos...</option>
                               ...
                               <option data-price="{{ $product->price }}" value="{{ $product->id }}">{{ $product->description }}</option> 
                               @endforeach
                          </select>
                        </div>
...
$(function () {
	  ...
	  function checkStock(pro_id,qty){
        ...
        $.ajax({
			url: "{{ route('admin.orders.checkStock') }}",
			type: "post",
			dataType: 'json',
               async: false,
			...
			success: function(data){
				...
				if(data.status == 'no'){
					localStorage.setItem("avail_qty", data.qty);
					status = 0;
				}
			}
